...
def get_contests_challenges(contest_name):
    # contest_name=str(co)
    # query = db.session.query(ContestsModel).filter_by(contest_name=contest_name).first()
    # contest_id =query.id 
    # query_one =db.session.query(ContestsChallengesModel,ContestsModel,ChallengesModel).join(ContestsModel,ContestsModel.id == ContestsChallengesModel.contest_id).filter_by(ContestsChallengesModel.contest_id=contest_id).join(ChallengesModel,ChallengesModel.id = ContestsChallengesModel.challenge_id)
    # for i in query_one:
    #     arr.append({"contest_name":i.ContestsModel.contest_name})
    # return arr
    data = {}
    challenge_data={}
    # import pdb; pdb.set_trace()
    data_raw = db.engine.execute("select * from contests join contests_challenges on contests.id=contests_challenges.contest_id join challenges on contests_challenges.challenge_id=challenges.id where contests.contest_name='{}'".format(contest_name))
    names = [dict(row) for  row in data_raw]
    challenges_arr=[]
    for i in names:
        # print(i['start'].strftime("%m/%d/%Y"))
        ...
        challenge_data['created_at']=str(i['created_at'].strftime("%m/%d/%Y"))
...
def add_contest(data,contest_name):
    ...
    new_asset = ContestsModel(contest_name=contest_name,start=start,end=end,details=details,show_leaderboard=show_leaderboard,)
    save_changes(new_asset)
    contest_id = new_asset.id
    for i in challenges_ids:
        new_asset = db.engine.execute("insert into contests_challenges (challenge_id,contest_id) values ({},{})".format(i,contest_id))
        
        
# db.engine.execute(" select contests.contest_name  from contests join contests_challenges on contests.id=contests_challenges.contest_id join challenges on contests_challenges.challenge_id=challenges.id where contests.contest_name={}".format(contest_name))
# "xyx{}".format(varname)