...
class Contest(Resource):
    ...
    def post(self, contest_name):
        ...
        parent_data = dict(contest_name=data['contest_name'], start_date=data['start_date'], end_date=data['end_date'],
                           details=data['details'], show_leader_board=data['show_leader_board'], challenges_ids=data['challenges_ids'])
        # Add contest to database
        created = add_contest(parent_data, contest_name)
        if created:
            return {
